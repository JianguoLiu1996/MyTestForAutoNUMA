...
OUTPATH="/home/liujianguo/MyTestForAutoNUMA/memtier_benchmark_test/Redis_Experiment_L_OFF_4nd/"
CONFIG="memtier_benchmark_L_OFF_4nd"
...
#	sudo numactl --interleave=all redis-server /etc/redis/redis6381.conf
	...
	wait
	echo "====success starting redis-server===="
}
# 准备三个进程的数据
function prepareDatabase(){
	...
	memtier_benchmark -s $SERVERADDR -p 6379 \
		...
		--key-pattern P:P \
		--pipeline 32 \
		--key-prefix=memtier-benchmark-prefix-redistests > /dev/null &
	memtier_benchmark -s $SERVERADDR -p 6380 \
		...
		--key-pattern P:P \
		--pipeline 32 \
		--key-prefix=memtier-benchmark-prefix-redistests-keysize70-xxxxxxxxxxxxx- > /dev/null &
	memtier_benchmark -s $SERVERADDR -p 6381 \
		...
		--key-pattern P:P \
		--pipeline 32 \
		--key-prefix=memtier-benchmark-prefix-redistests-keysize100-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx- > /dev/null &
	wait
